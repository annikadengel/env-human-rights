...
# diagnostics
stripplot(imp_panel, wdi_fossil) # EXAMINE

bwplot(imp_panel, wdi_fossil,
          xlab = "Imputed and Observed Values", 
          ylab = "Fossil Fuel Consumption (WDI)",
          main = "Imputation Diagnostics")
...
# [1] "env_index_gls" "v2x_libdem" "capacity" "NY.GNP.PCAP.CD" "SP.POP.TOTL" "v2x_rule" 
stripplot(gs_imp_panel, env_index_gls)
stripplot(gs_imp_panel, v2x_libdem)
stripplot(gs_imp_panel, capacity)
stripplot(gs_imp_panel, NY.GNP.PCAP.CD) # log-transformed
stripplot(gs_imp_panel, SP.POP.TOTL) # log-transformed
...
# Set grayscale theme for publication-style visuals

# Capture each lattice plot properly for grid.arrange
p1 <- grid.grabExpr(stripplot(gs_imp_panel, env_index_gls,
                              main = "Convergence: Environmental Index",
                              xlab = "Imputation Number",
                              ylab = "Standardized Index Value"))

p2 <- grid.grabExpr(stripplot(gs_imp_panel, v2x_libdem,
                              main = "Convergence: Liberal Democracy (V-Dem)",
                              xlab = "Imputation Number",
                              ylab = "Liberal Democracy Score"))

p3 <- grid.grabExpr(stripplot(gs_imp_panel, capacity,
                              main = "Convergence: State Capacity",
                              xlab = "Imputation Number",
                              ylab = "State Capacity Index"))

p4 <- grid.grabExpr(stripplot(gs_imp_panel, NY.GNP.PCAP.CD,
                              main = "Convergence: Log GNP per Capita",
                              xlab = "Imputation Number",
                              ylab = "Log(GNP per Capita)"))

p5 <- grid.grabExpr(stripplot(gs_imp_panel, SP.POP.TOTL,
                              main = "Convergence: Log Population",
                              xlab = "Imputation Number",
                              ylab = "Log(Total Population)"))

p6 <- grid.grabExpr(stripplot(gs_imp_panel, v2x_rule,
                              main = "Convergence: Rule of Law (V-Dem)",
                              xlab = "Imputation Number",
                              ylab = "Rule of Law Score"))
...
# examining the medium capacity stuff further...
# compare classifications side by side
comparison <- data.frame(
  country = imp_data_gs$country_name,
  old = imp_data_gs$capacity_group,
  new = imp_data_gs$capacity_strata_window
)

comparison[comparison$old != comparison$new, ]

consistent_medium <-imp_data_gs |>
  filter(capacity_group == "medium" & capacity_strata_window == "medium")

gsynth_results_mc_consistent <- gsynth(
  formula = env_index_gls ~ treatment + v2x_libdem + log_pop + log_gnp + capacity,
  data = consistent_medium,
  index = c("ccodecow", "year"),
  force = "two-way",
  CV = TRUE,
  r = c(0, 5),
...
print(gsynth_results_mc_consistent)

ever_medium <-imp_data_gs |>
  filter(capacity_group == "medium" | capacity_strata_window == "medium")

gsynth_results_mc_ever <- gsynth(
  formula = env_index_gls ~ treatment + v2x_libdem + log_pop + log_gnp + capacity,
  data = ever_medium,
  index = c("ccodecow", "year"),
  force = "two-way",
...
print(gsynth_results_mc_ever)
#Average Treatment Effect on the Treated:
#  Estimate   S.E. CI.lower CI.upper p.value
#ATT.avg   0.2669 0.1065  0.05827   0.4756 0.01217
...
model_ceilling2 <- feols(env_index_gls ~ capacity+ log_gnp + v2x_libdem + log_pop | factor(year), data = imp_data_treat) # continous not categorical
...
pre_recog_data$year_c <- pre_recog_data$year - min(pre_recog_data$year) # centering year
model_interact <- lm(env_index_gls ~ year_c * capacity_group + log_gnp, data = pre_recog_data)
...
unique(sig_countries$country_name)
unique(imp_data_gs$country_name[capacity_group == "medium"])
